...
trait JavaRDDLike[T, This <: JavaRDDLike[T, This]] extends Serializable {
  ...
  def map[R](f: JFunction[T, R]): JavaRDD[R] =
    new JavaRDD(rdd._map(f)(fakeClassTag))(fakeClassTag)
  ...
  def mapToPair[K2, V2](f: PairFunction[T, K2, V2]): JavaPairRDD[K2, V2] = {
    ...
    new JavaPairRDD(rdd._map[(K2, V2)](f)(cm))(fakeClassTag[K2], fakeClassTag[V2])
  }
  ...
  def flatMap[U](f: FlatMapFunction[T, U]): JavaRDD[U] = {
    ...
    JavaRDD.fromRDD(rdd._flatMap(fn)(fakeClassTag[U]))(fakeClassTag[U])
  }
  ...
  def flatMapToDouble(f: DoubleFlatMapFunction[T]): JavaDoubleRDD = {
    ...
    new JavaDoubleRDD(rdd._flatMap(fn).map(_.doubleValue()))
  }
  ...
  def flatMapToPair[K2, V2](f: PairFlatMapFunction[T, K2, V2]): JavaPairRDD[K2, V2] = {
    ...
    def cm: ClassTag[(K2, V2)] = implicitly[ClassTag[(K2, V2)]]
    JavaPairRDD.fromRDD(rdd._flatMap(fn)(cm))(fakeClassTag[K2], fakeClassTag[V2])
  }
  ...
  def glom(): JavaRDD[JList[T]] =
    new JavaRDD(rdd.glom()._map(_.toSeq.asJava))
   ...
   * operator.
   */
  def reduce(f: JFunction2[T, T, T]): T = rdd._reduce(f)
